...
def agregar_experimentos():
    ...
    while True:
        fecha_str = input("Introduce la fecha del experimento (YYYY-MM-DD): ")
        try:
    ...
    try:
        ...
        print("Los resultados deben ser valores numéricos.")
        return
...
def comparar_experimentos():
    ...
    if exp1 and exp2:
        ...
        print(f"Desviación estándar de {exp1.nombre}: {statistics.stdev(exp1.resultados) if len(exp1.resultados) > 1 else 0}")
        print(f"Desviación estándar de {exp2.nombre}: {statistics.stdev(exp2.resultados) if len(exp2.resultados) > 1 else 0}")
    else:
